 # tests/test_users.py
import pytest
def user_payload(uid=1, name="Liam", email="g00420041@atu.ie", age=21, sid="S1234567"):
...
def test_update_user_ok(client): 
    client.post("/api/users", json=user_payload(uid=20, name="Liam", email="G00420041@atu.ie", age=21))   
    updated = user_payload(uid=20, name="Liam", email="G00420041@atu.ie", age=21)   
    r = client.put("/api/users/20", json=updated)
    assert r.status_code == 200
    
    data = r.json()
    assert data["name"] == "Liam"
    assert data["email"] == "G00420041@atu.ie"
    assert data["age"] == 21


def test_update_user_404(client): #False user    
    updated = user_payload(uid=321, name="Fake", email="Fakeuser@atu.ie")
    r = client.put("/api/users/321", json=updated)
    assert r.status_code == 404
